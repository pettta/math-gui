...
//> imgui_draw
	...
	if (_imguiBackend && _imguiRenderer)
	{
		_imguiBackend->setFrameResources(cmd, _swapchainImageViews[swapchainImageIndex], _swapchainExtent);
		_imguiRenderer->renderFrame([this](ImGuiRenderer::FrameState& state) {
			(void)state;
			if (ImGui::Begin("Background"))
			{
				...
				{
					const int maxIndex = static_cast<int>(backgroundEffects.size() - 1);
					currentBackgroundEffect = std::clamp(currentBackgroundEffect, 0, maxIndex);
					ComputeEffect& selected = backgroundEffects[static_cast<size_t>(currentBackgroundEffect)];
...
void VulkanEngine::rebuild_swapchain()
{
	...
	if (_imguiBackend)
	{
		...
		const uint32_t minImageCount = std::max(imageCount, 2u);
...
void VulkanEngine::init_imgui()
{
	...
	_imguiBackend = std::make_unique<VulkanImguiBackend>(
		_window,
		_instance,
		_chosenGPU,
		_device,
	...
	_imguiRenderer = std::make_unique<ImGuiRenderer>(*_imguiBackend);
